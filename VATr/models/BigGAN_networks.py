...
class WDiscriminator(nn.Module):
    ...
    def __init__(self, resolution, n_classes, output_dim, D_ch=64, D_wide=True, D_kernel_size=3, D_attn='64',
        ...
        self.attention = D_attn
        # Number of classes
        self.n_classes = n_classes # n_classes = self.args.vocab_size
        # output_dim
        self.output_dim = output_dim # output_dim = self.args.num_writers
        ...
        self.feature_compress_flag = True


    def forward(self, x, y=None, **kwargs):


        if self.feature_compress_flag == True:
            h = self.feature_compress(x, y)

        # Stick x into h for cleaner for loops without flow control
        elif self.feature_compress_flag == False:
            h = x
            # Loop over blocks
            for index, blocklist in enumerate(self.blocks):
                for block in blocklist:
        ...
        # @Ym focal loss
        loss = self.foacl_loss(out, y.long())

        return loss

    def feature_compress(self, x, y):
        x = self.get_backbone_x(x)

        if self.feature_compress_flag: # epoch limtation will coming late?
            x = fea_com(x, y, self.output_dim)
    ...
    def _get_module(self, module_type = "GAP"):
        if module_type == "GAP":
            module = GAP()
    ...
    def get_backbone_x(self, x, y=None):
        # Stick x into h for cleaner for loops without flow control
        h = x
        # block_output = []
        # Loop over blocks
...
class FocalLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.gamma = 2 #hyper-parameter
        self.sigmoid = nn.Sigmoid()
        self.num_classes = 339
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        weight = torch.FloatTensor(torch.ones(targets.shape[0])).to(targets.device)
...
#         logpt = logpt.view(-1)
#         pt = Variable(logpt.data.exp())  # from torch.autograd import Variable
#
