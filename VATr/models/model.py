...
class CorrectorWD(nn.Module):

    def __init__(self, args):
        super(CorrectorWD, self).__init__()

        self.args = args
        if self.args.is_seq: INP_CHANNEL = 1
        self.feat_model_path = 'files/resnet_18_pretrained.pth'
        pre_trained = True

        ## network
        self.FWL_Feat_Encoder = models.resnet18(weights='ResNet18_Weights.DEFAULT')
        self.FWL_Feat_Encoder.conv1 = nn.Conv2d(INP_CHANNEL, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.FWL_Feat_Encoder.fc = nn.Identity()
        self.FWL_Feat_Encoder.avgpool = nn.Identity()

        # load pre-trained model
        if pre_trained:
            print('CorrectorWD, Loading...', self.feat_model_path)
            assert os.path.exists(self.feat_model_path)
            checkpoint = torch.load(self.feat_model_path)
            checkpoint['model']['conv1.weight'] = checkpoint['model']['conv1.weight'].mean(1).unsqueeze(1)
            miss, unexp = self.FWL_Feat_Encoder.load_state_dict(checkpoint['model'], strict=False)
            assert unexp == ['fc.weight', 'fc.bias']


    def forward(self, x_fake, x_real, y):
        fake_space = self.tensor2Space(x_fake)
        real_space = self.tensor2Space(x_real)
        print("run @ CorrectorWD, fake_space.shape: {}".format(fake_space.shape))
        print("run @ CorrectorWD, real_space.shape: {}".format(real_space.shape))


    def tensor2Space(self, x):
        # FWL get feature
        ST = x
        Feat_ST = self.FWL_Feat_Encoder(ST)
        return Feat_ST
...
class VATr(nn.Module):
    ...
    def backward_D_WL(self):
        ...
        if not self.args.no_writer_loss: # self.args.no_writer_loss FALSE
            # print("@")
            # print(self.input['wcl'])
